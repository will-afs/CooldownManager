...
def test_get_token_view(app):
    ...
    for i in range(0,3):
        ...
        threads[i] = threading.Thread(target=get_token, args=(clients[i], "/", i, request_times, responses, response_times), daemon=True)
    ...
    for i in range(0,3):
        ...
        assert response_times[i] - request_times[i] == pytest.approx(expected_time_delta, abs=(i+1)*network_related_tolerance)
