...
class TestCoolDownManager(unittest.TestCase):
    ...
    def test_cooldown(self):
        ...
        self.assertAlmostEqual(time_1-time_0, self.ref_cooldown_delay, delta=self.tolerance)

    def test_get_token(self):
        run_thread = threading.Thread(target=self.cooldownmanager._run, args=(), daemon=True)
        run_thread.start()
        ...
        self.assertAlmostEqual(float(time_1-time_0), float(0), delta=float(self.tolerance))
        ...
        self.assertLessEqual(time_2-time_1, self.ref_cooldown_delay) # Otherwise this test would be pointless
        self.assertEqual(token_2, True)
        self.assertGreaterEqual(time_3-time_1, self.ref_cooldown_delay)
        self.assertAlmostEqual(time_3-time_1, self.ref_cooldown_delay, delta=self.tolerance)
